...
async def test_create_folder_path_success(onedrive_service):
    ...
    with patch.object(onedrive_service, "_create_or_get_folder", side_effect=mock_create_or_get):
...
async def test_create_or_get_folder_exists(onedrive_service):
    ...
    mock_response.json.return_value = {
        "value": [
            {"name": "TestFolder", "id": "existing_id", "folder": {}}
        ]
    }
    
    with patch.object(onedrive_service, "_make_graph_request", return_value=mock_response):
...
async def test_create_or_get_folder_creates_new(onedrive_service):
    ...
    with (
        patch.object(onedrive_service, "_make_graph_request", return_value=mock_list_response),
        patch.object(onedrive_service, "_create_single_folder", return_value="new_id") as mock_create,
    ):
...
async def test_create_folder_if_not_exists(onedrive_service):
    ...
    with patch.object(onedrive_service, "_create_or_get_folder", return_value="folder_id") as mock:
...
@pytest.mark.asyncio
async def test_upload_file_with_organize_by_year(onedrive_service, mock_edition, tmp_path):
    ...
    with (
        ...
        patch.object(onedrive_service, "_make_graph_request", return_value=mock_response),
    ):
        result = await onedrive_service.upload_file(
            str(test_file), mock_edition, folder_name="TestFolder", organize_by_year=True
        )
...
async def test_upload_file_exception(onedrive_service, mock_edition, tmp_path):
    ...
    with (
        patch.object(onedrive_service, "authenticate", return_value=True),
        patch.object(onedrive_service, "create_folder_path", side_effect=Exception("Unexpected error")),
    ):
        result = await onedrive_service.upload_file(
            str(test_file), mock_edition, folder_name="TestFolder"
        )
...
async def test_list_files_with_folder(onedrive_service):
    ...
    with (
        patch.object(onedrive_service, "create_folder_if_not_exists", return_value="folder_id"),
        patch.object(onedrive_service, "_make_graph_request", return_value=mock_response),
    ):
...
async def test_list_files_folder_not_found(onedrive_service):
    ...
    with patch.object(onedrive_service, "create_folder_if_not_exists", return_value=None):
